...
const RightPanel = () => {
  ...
  useEffect(() => {
    //console.log(similarExecutiveList,'similarExecutiveList')
  ...
  const viewExecutiveDetails = (id, name) => {
  ...
  const renderSimilarExecutiveList = () => {
    ...
    for (let i = 0; i < similarCount; i++) {
      similarList.push(
        <div
          className={
            i > 0 ? "row mt-3 pl-2 brdr-b pb-3" : "row brdr-b pl-2 pb-3"
          }
        >
          <div
          ...
          >
            {similarExecutiveList[i]?.firstname?.[0]}
            {similarExecutiveList[i]?.lastname?.[0]}
          </div>
          <div className="col">
            <div>
              <Link
                className="font-weight-bold fs-14 text-dark"
                to={`/executive-details/${
                  similarExecutiveList[i]?.id
                }/${similarExecutiveList[i]?.fullname.replaceAll(
                  " ",
                  "-",
              ...
              >
                {similarExecutiveList[i]?.fullname
                  ? similarExecutiveList[i].fullname
                  : similarExecutiveList[i]?.firstname +
                    " " +
                    similarExecutiveList[i]?.lastname}
              </Link>
              {/* <a
                    className="font-weight-bold fs-14 text-dark"
                    title=""
                    onClick={() => viewExecutiveDetails(similarExecutiveList[i]?.id,similarExecutiveList[i]?.fullname)}
  ...
  const checkRelavantExecutive = (flag, thumbStatus) => {
    //checkRelavantExecutive
  ...
  const handleErrorForm = () => {
    //console.log("click report an error")
    ...
    <>
      ...
      <div className="btn btn-outline-secondary mt-3 pr-2 "><i className="right-icons las la-flag" aria-hidden="true"></i>Report An Error</div>
        ...
        <div className="card shadow  mb-4">
          <div className="card-header font-weight-bold">Company Info</div>
          <div className="card-body c-info ">
            <div className="c-info-detail">
              <div className=" executive-company mr-2 img-responsive">
                <img
                  src={companyDetails?.companyLogoUrl || defaultCompanyLogo}
                  onError={(e) => {
              ...
              <div className="c-info-desc">
                <div>
                  <a
                    className="font-weight-bold fs-16 text-dark"
                    title=""
                    href="#"
                    target=""
                  >
            ...
            <div className="d-flex">
              <div className="row pb-3">
                <div className=" pl-3">
                  <div className="fs-12">
                    <h6>
                      <i className=" fs-16 mr-2  la la-phone text-black"></i>
                      Phone
                    </h6>
                    <div className="pl-4 ">{companyDetails?.phoneNo}</div>
                  </div>
                  <div className="fs-12">
                    <h6>
                      <i className=" fs-16 mr-2  la la-map-marker text-black"></i>
                      Location
                    </h6>
                    <div className="pl-4 ml-1">{companyDetails?.address}</div>
                  </div>
                  <div className="fs-12">
                    <h6>
                      <i className=" fs-16 mr-2  la la-users text-black"></i>
                      Employees Ranges
                    </h6>
                    <div className="pl-4 ml-1">
                      {companyDetails?.range?.name}
                    </div>
                  </div>
                  <div className="fs-12">
                    <h6>
                      <i className=" fs-16 mr-2  las la-industry text-black"></i>
                      Industry
                    </h6>
                    <div className="pl-4 ml-1">
                  ...
                  <div className="fs-12">
                    <h6>
                      <i className=" fs-16 mr-2  la la-globe text-black"></i>
                      Website
                    </h6>
                    <div className="pl-4 ml-1">{companyDetails?.wedsite}</div>
                  </div>
                  <div className="fs-12">
                    <h6>
                      <i className=" fs-16 mr-2  la la-city text-black"></i>
                      Company Type
                    </h6>
                    <div className="pl-4 ml-1">
                  ...
                  <div className="fs-12">
                    <h6>
                      <i className=" fs-16 mr-2  la la-money-check text-black"></i>
                      Revenue Range
                    </h6>
                    <div className="pl-4 ml-1">
                      {" "}
                      {companyDetails?.ravenue?.name}
                    </div>
                  </div>
                </div>
              </div>
        ...
        <div className="card shadow mt-2 mb-4">
          <div className="card-header font-weight-bold">Similar Executive</div>
        ...
        <div className="card shadow descbox1 mb-3">
          <div>
            Is this executive data relevant to you?
            {checkExecutiveStatus === 1 ? (
              <a
                id=""
                role="button"
                data-toggle=""
              ...
              >
                <i
                  className="right-icons fa fa-thumbs-up"
                  aria-hidden="true"
                ></i>
              </a>
            ) : (
        ...
        <div className="buttons-container">
          <div
            className="btn btn-outline-secondary  w-99 pr-2 "
            onClick={handleErrorForm}
          >
            <i className="right-icons las la-flag" aria-hidden="true"></i>Report
            An Error
          </div>
        </div>
      </div>
